...
with open(f'{simdir}/model{modelnumber:02n}/AUX/radmc3d.inp','w+') as f:
    ...
    f.write('istar_sphere = 0\n')          ## while finite stars *would* be more physical, this matches the 2d code more closely for now
    f.write('iranfreqmode = 1\n')          ##
    f.write('modified_random_walk = 1\n')  # this analytically determines outward travel from high-τ cells

## Make the wavelength input file

with open('frequency.inp') as f: ## read in the 2d frequencies grid
...
wavs = np.array(wavelengths) ## create 'wavs' array for many later uses
...
## constant star control data
pstar = [0,0,0]
nonzero_intensities = np.full(len(wavs), 1e-300)

#### ## this bloack of code *would* be used to elect more accurate apertures
...
## write the scattering opacities data file
with open('dustopac_1.inp') as f: ## read the 2d control file
    opacs = f.readlines()
absp = opacs[2:102]
scat = opacs[102:]
...
for i in range(1,len(mpars[0])):
    ...
    #cv = 2*np.pi*[(1/3)*(r_walls[i]**3-r_walls[i+1]**3)]*[np.cos(theta_walls[i])-np.cos(theta_walls[i+1])]
...
#     rhoenv = np.where(rr >= renv_in*au, rhoenv, 0) ## if not within core bounds, rho -> 0
    ...
    bbody = models.BlackBody(temperature=tstar*u.K)
    inten = bbody(wavs*u.um)*np.pi*((rstar*rs)/pc)**2
    outspec = np.maximum(inten.value, nonzero_intensities)
